...
6. **Source every concrete claim.** Before writing any milestone, number, date, status, decision, or attribution into a note or a stakeholder-facing message, name where it came from. If you can't, hedge it or leave it out. Don't translate operational language ("going live," "should be ready," "looks good") into milestone language ("launched," "shipped," "done"). Don't erode someone's hedge as it travels from a transcript to a note to a report. Vault entries become next month's "facts," so write only what the source supports. Full rule in `skills/_shared/source-discipline.md`.
7. **Verify the current date before any dated work.** Run `date '+%Y-%m-%d %H:%M %Z'` to get the real date before routing to a daily note, framing "today," calculating a date range, or stamping a report. Don't rely on the date in your session context, which is set when the session starts and can be stale by the time you use it. If live data (file timestamps, message dates) conflicts with the assumed date, trust the live data.
