...
**How to install one.** Setup happens inside your AI tool, not inside the vault. The easiest path is the one you're already using: open a chat with Claude (or whatever AI tool you've connected to your vault) and ask it directly. Something like "I want to connect Gmail to you, can you walk me through setting up an MCP?" It'll tell you what to install, where the config goes, and what to do if something doesn't work. You're talking to the same AI that's about to use the connection, so it knows what it needs.

If you'd rather start with docs, the official sources are the right place because the install steps change as the ecosystem matures. The Anthropic docs at [docs.anthropic.com](https://docs.anthropic.com) cover Claude Code and Claude Cowork. Most major services now publish their own MCPs with install instructions on their developer pages. A useful shortcut: google the service plus "MCP" (try "gmail mcp," "google calendar mcp," "notion mcp") and you'll land on the right setup page.
